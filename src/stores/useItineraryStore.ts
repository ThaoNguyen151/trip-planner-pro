...
import type { ItineraryDay, ItineraryStore } from "@/types/itinerary"
  ...
  persist(
    (set) => ({
      ...
      addActivity: (day, activity) =>
        set((state) => ({
          days: state.days.map((d) =>
            d.day === day
              ? { ...d, activities: [...d.activities, { id: genId(), ...activity }] }
              : d,
          ),
        })),

      updateActivity: (day, activityId, updates) =>
        set((state) => ({
          days: state.days.map((d) =>
            d.day === day
              ? {
      ...
      toggleCollapse: (day) =>
        set((state) => ({
          days: state.days.map((d) =>
            d.day === day ? { ...d, collapsed: !d.collapsed } : d,
          ),
        })),
    }),
    {
