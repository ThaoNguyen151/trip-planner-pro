...
export function useItineraryFilters() {
  ...
  const filteredDays = useMemo<ItineraryDay[]>(() => {
    ...
    return days
      ...
      .map((d) => ({
        ...d,
        collapsed: filterActive ? false : d.collapsed,
        activities: d.activities.filter((a) => {
          ...
          if (filters.priority !== "All Priorities" && a.priority !== filters.priority) return false
          return true
        }),
      }))
  ...
  return { filters, dateOptions, hasActiveFilters, filteredDays, clearFilters, updateFilter }
}
